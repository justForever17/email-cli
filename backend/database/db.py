        ...
        with cls._lock:
            if cls._instance is None:
                ...
                if getattr(sys, 'frozen', False):
                    # 打包后的EXE运行环境：将数据持久保存在同目录下的 backend/data 目录中
                    exe_dir = os.path.dirname(sys.executable)
                    data_dir = os.path.join(exe_dir, 'backend', 'data')
                    os.makedirs(data_dir, exist_ok=True)
                    db_path = os.path.join(data_dir, 'email-cli.db')
                else:
                    db_path = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), 'data', 'email-cli.db')
