    ...
    def fetch_emails(email_address, password, server, port=993, use_ssl=True, folder="INBOX", callback=None, last_check_time=None):
        ...
        try:
            ...
            for f_idx, current_folder in enumerate(folders_to_check):
                ...
                try:
                    # 选择邮件文件夹
                    logger.info(f"选择文件夹 {current_folder}")
                    if callback:
                        callback(20, f"正在选择文件夹 {current_folder}")
                    ...
                    if last_check_time:
                        ...
                        if date_str:
                            ...
                            logger.info(f"在 {current_folder} 中获取自 {date_str} 以来的新邮件")
                    
                    _, messages = mail.search(None, search_criteria)
                    message_numbers = messages[0].split()
                    total_messages = len(message_numbers)
                    
                    logger.info(f"在 {current_folder} 中找到 {total_messages} 封邮件")
                    ...
                    for i, num in enumerate(message_numbers):
                        try:
                            # 更新进度
                            progress = int(base_progress + ((i + 1) / total_messages) * (70 / total_folders)) if total_messages > 0 else base_progress
                            if callback:
                                callback(progress, f"正在处理 {current_folder} 第 {i + 1}/{total_messages} 封邮件")
                            ...
                            # 解析邮件，带上当前文件夹信息
                            mail_record = parse_email_message(msg, current_folder)
                            if mail_record:
                                # 添加一些额外信息用于去重判断
