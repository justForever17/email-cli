...
class OutlookMailHandler:
    ...
    def fetch_emails(email_address, access_token, folder="INBOX", callback=None, last_check_time=None):
        ...
        Args:
            ...
            folder: 邮件文件夹，默认为收件箱。如果为"INBOX"，则会自动尝试检查垃圾邮件文件夹
            callback: 进度回调函数
                    ...
                    try:
                        ...
                        if status == 'OK' and folder_list:
                            for f in folder_list:
                                ...
                                if 'junk' in f_lower or 'spam' in f_lower or '垃圾' in f_lower:
                                    if f_name not in folders_to_check:
                                        folders_to_check.append(f_name)
                                        logger.info(f"自动发现垃圾文件夹: {f_name}，已添加到检查列表")
                                        break
                    except Exception as e:
                        logger.error(f"探测垃圾邮件文件夹时出错: {str(e)}")
                
                total_folders = len(folders_to_check)
                for f_idx, current_folder in enumerate(folders_to_check):
                    logger.info(f"正在处理文件夹: {current_folder} ({f_idx + 1}/{total_folders})")
                    
                    try:
                        # 选择文件夹
                        select_status, _ = mail.select(current_folder)
                        if select_status != 'OK':
                            logger.error(f"选择文件夹 {current_folder} 失败: {select_status}")
                            continue
                            
                        # 计算基础进度
                        base_progress = 20 + int((f_idx / total_folders) * 70)
                        callback(base_progress, current_folder)
                        ...
                        if last_check_time:
                            ...
                            logger.info(f"在 {current_folder} 中搜索 {search_date} 之后的邮件")
                        ...
                        if status != 'OK':
                            logger.error(f"在 {current_folder} 搜索邮件失败: {status}")
                            continue
                        ...
                        logger.info(f"在 {current_folder} 中找到 {total_mails} 封邮件")
                        
                        # 处理每封邮件
                        for i, mail_id in enumerate(mail_ids):
                            # 更新进度
                            sub_progress = int(base_progress + (i / total_mails) * (70 / total_folders)) if total_mails > 0 else base_progress
                            callback(sub_progress, current_folder)
                            ...
                            try:
                                ...
                                mail_records.append({
                                    ...
                                    'content': content,
                                    'mail_key': mail_key,
                                    'folder': current_folder  # 保存当前文件夹，例如 INBOX 或 Junk Email
                                })
                                
                            except Exception as e:
                                logger.error(f"处理邮件ID {mail_id} 时出错: {str(e)}")
                    except Exception as folder_err:
                        logger.error(f"处理文件夹 {current_folder} 时发生异常: {str(folder_err)}")
