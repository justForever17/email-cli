...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("email-cli.log", encoding='utf-8'),
        logging.StreamHandler()
    ]
)
logger = logging.getLogger('Email-cli')
...
# JWT密钥
JWT_SECRET = os.environ.get('JWT_SECRET_KEY', 'email_cli_secret_key')
...
def health_check():
    """健康检查接口"""
    return jsonify({'status': 'ok', 'message': 'Email-cli服务正在运行'})
...
@token_required
def send_email(current_user, email_id):
    """发送邮件"""
    try:
        # 1. 验证发件邮箱的所有权和存在性
        email_info = db.get_email_by_id(email_id, None if current_user['is_admin'] else current_user['id'])
        ...
        if not recipient or not subject or not body:
            return jsonify({'error': '收件人、主题和正文内容不能为空'}), 400
            
        logger.info(f"用户 {current_user['username']} 尝试通过邮箱 {email_info['email']} 发送邮件给 {recipient}")
        ...
        logger.error(f"通过 API 发送邮件失败: {str(e)}")
...
def parse_args():
    """解析命令行参数"""
    parser = argparse.ArgumentParser(description='Email-cli')
...
if __name__ == '__main__':
    try:
        ...
        # 启动Flask应用
        logger.info(f"Email-cli启动于 http://{args.host}:{args.port}")
